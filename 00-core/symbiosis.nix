...
let
  ...
  # 4GB ist unser hartes Minimum für einen "ernstzunehmenden" Server
  isLowRam = ramGB <= 4;
in
{
  ...
  nix.settings = {
    # Bei 4GB oder weniger: Nur 1 Job, um OOM zu vermeiden
    max-jobs = if isLowRam then lib.mkForce 1 else lib.mkForce 2;
    cores = if isLowRam then lib.mkForce 2 else lib.mkForce 4;
  };
  
  # ZRAM als Lebensretter für 4GB Systeme
  zramSwap.enable = isLowRam;
  zramSwap.memoryPercent = 50; # 50% des RAMs als komprimierter Swap

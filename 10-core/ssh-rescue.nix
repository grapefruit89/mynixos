 ...
 * LAYER:        10
 * STATUS:       Stable
 * INTEGRITY:    SHA256:348274c02000d217d29645556122b22fca0fe67efc9c3c8f7c34764a3cf28307
 */
...
in
{
  ...
  systemd.services.ssh-recovery-window = {
    ...
    documentation = [ "https://github.com/grapefruit89/mynixos/blob/main/10-core/ssh-rescue.nix" ];
...
EOF
  ...
  programs.bash.interactiveShellInit = lib.mkIf (user == "moritz") ''
    ...
    if [ -n "$SSH_CONNECTION" ]; then
      if ${recoveryStatus}/bin/ssh-recovery-status 2>/dev/null; then
        ...
        echo "   3. Füge in /etc/nixos/10-core/users.nix hinzu"
  ...
  systemd.services.ssh-rescue-info = {
    ...
    script = ''
      ...
      ║  3. Public Key in /etc/nixos/10-core/users.nix eintragen      ║
